...
本目录是 TravelAgent 单仓库中的 Go 后端模块。当前版本聚焦知识文档摄取：上传文件、按内容去重、显式分块、调用 OpenAI 兼容的 Embedding API，并把文档、分块和 1536 维向量写入 PostgreSQL/pgvector。

项目采用“Go 单服务模块 + 模块化单体 + 轻量 DDD”。后端命令都从 `backend/` 目录执行；前端、Docker 编排和仓库级开发工具位于它的同级目录。
...
Docker 资产独立位于仓库根目录 `docker/`，详细说明见 [`../docker/README.md`](../docker/README.md)。Compose 从仓库根目录执行，并通过根级 `.dockerignore` 控制构建上下文；Dockerfile 只复制本目录中的 Go 依赖清单和生产源码。
...
默认栈包含 PostgreSQL/pgvector 与后端服务；增加 `--profile s3` 会同时启动 MinIO。空库首次初始化挂载 `backend/migrations/000001_rag_baseline.sql`，但应用自身不会自动运行任何迁移。baseline 包含重建语句，只能用于全新空数据库。
